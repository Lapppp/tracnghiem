...
namespace App\Http\Controllers\Backend\BGBanner;
...
use App\Repositories\Banners\BGBannerRepository;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\View;
use Illuminate\Support\Str;
use Intervention\Image\Facades\Image as ImageIntervention;

class BGBannerController extends BackendController
{
    ...
    public function __construct( BGBannerRepository $bannerRepository )
    {
        parent::__construct();
    ...
    public function index( Request $request )
    {
        ...
        return view('components.backend.bgbanner.index', $this->data);
    }

    public function create()
    {
        $this->data['isEdit'] = 0;
        return view('components.backend.bgbanner.create', $this->data);
    }
    ...
    public function store( BannerCreateRequest $request )
    {
        ...
        if ( !$post ) {
            return redirect()->route('backend.bgbanner.index')->with('error', 'Server đang bận không thể tạo');
        }
        ...
        return redirect()->route('backend.bgbanner.index')->with('success', 'Đã tạo tài thành công');
    }
    ...
    public function edit( $id )
    {
        ...
        if ( !$post ) {
            return redirect()->route('backend.bgbanner.index')->with('error', 'Không tìm thấy dữ liệu');
        }
        $this->data['isEdit'] = 1;
        return view('components.backend.bgbanner.create', $this->data);
    }
    ...
    public function update( BannerCreateRequest $request, $id )
    {
        ...
        return redirect()->route('backend.bgbanner.index')->with('success', 'Đã cập nhật thành công');
    }
