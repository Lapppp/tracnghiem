...
class GuideController extends BackendController
{
    ...
    public function index( Request $request )
    {
        ...
        $post = $this->postRepository->getAll(['module_id'=>[ModuleType::Guide]], 20);
        $this->data['title'] = 'Guide';
        $this->data['items'] = $post;
        $total = !empty($post->total()) ? $post->total() : 0;
        $perPage = !empty($post->perPage()) ? $post->perPage() : 2;

        $page = !empty($request->page) ? $request->page : 1;
        $url = route('backend.guide.index') . '?' . Arr::query($params);
        $this->data['pager'] = PaginationHelper::BackendPagination($total, $perPage, $page, $url);
        return view('components.backend.guide.index', $this->data);
    }

    public function create()
    {
        $this->data['category'] = $this->categoryRepository->getAll(['module_id'=>[ModuleType::Guide]]);
        $this->data['isEdit'] = 0;
        return view('components.backend.guide.create', $this->data);
    }
    ...
    public function store( NewsCreateRequest $request )
    {
        ...
        $params['module_id'] = ModuleType::Guide;
        $post = $this->postRepository->create($params);
        if ( !$post ) {
            return redirect()->route('backend.guide.index')->with('error', 'Server đang bận không thể tạo');
        }
        ...
        return redirect()->route('backend.guide.index')->with('success', 'Đã tạo  thành công');
    }
    ...
    public function edit( $id )
    {
        ...
        if ( !$post ) {
            return redirect()->route('backend.guide.index')->with('error', 'Không tìm thấy dữ liệu');
        }
        $this->data['isEdit'] = 1;
        $this->data['category'] = $this->categoryRepository->getAll(['module_id'=>[ModuleType::Guide]]);
        return view('components.backend.guide.create', $this->data);
    ...
    public function update( NewsCreateRequest $request, $id )
    {
        ...
        return redirect()->route('backend.guide.index')->with('success', 'Đã cập nhật thành công');
    }
    ...
    {
        $params = $request->only(['username', 'password']);
        $post = $this->categoryRepository->getAll(['module_id'=>[ModuleType::Guide]], 20);
        ...
        $page = !empty($request->page) ? $request->page : 1;
        $url = route('backend.guide.category') . '?' . Arr::query($params);
        $this->data['pager'] = PaginationHelper::BackendPagination($total, $perPage, $page, $url);
        return view('components.backend.guide.category', $this->data);
    }
    ...
    public function createCategory()
    {
        $this->data['category'] = $this->categoryRepository->getAll(['module_id'=>[ModuleType::Guide]]);
        $this->data['isEdit'] = 0;
        return view('components.backend.guide.category-create', $this->data);
    }
    ...
    public function storeCategory( CategoryRequest $request )
    {
        ...
        if ( !$post ) {
            return redirect()->route('backend.guide.category')->with('error', 'Server đang bận không thể tạo');
        }

        return redirect()->route('backend.guide.category')->with('success', 'Đã tạo tài thành công');
    }
    ...
    public function updateCategory( CategoryRequest $request, $id )
    {
        ...
        if ( !$post ) {
            return redirect()->route('backend.guide.category')->with('error', 'Không tìm thấy dữ liệu');
        }
        $post->update($params);

        return redirect()->route('backend.guide.category')->with('success', 'Đã cập nhật thành công');
    }
