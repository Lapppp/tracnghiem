...
class WisdomController extends BackendController
{
    ...
    public function index( Request $request )
    {
        ...
        $post = $this->postRepository->getAll(['module_id'=>[ModuleType::Wisdom]], 20);
        ...
        $url = route('backend.wisdom.index') . '?' . Arr::query($params);
        $this->data['pager'] = PaginationHelper::BackendPagination($total, $perPage, $page, $url);
        return view('components.backend.wisdom.index', $this->data);
    }

    public function create()
    {
        $this->data['category'] = $this->categoryRepository->getAll(['module_id'=>[ModuleType::Wisdom]]);
        $this->data['isEdit'] = 0;
        return view('components.backend.wisdom.create', $this->data);
    }
    ...
    public function store( NewsCreateRequest $request )
    {
        ...
        $params['module_id'] = ModuleType::Wisdom;
        $post = $this->postRepository->create($params);
        if ( !$post ) {
            return redirect()->route('backend.wisdom.index')->with('error', 'Server đang bận không thể tạo');
        }
        ...
        return redirect()->route('backend.wisdom.index')->with('success', 'Đã tạo tài thành công');
    }
    ...
    public function edit( $id )
    {
        ...
        if ( !$post ) {
            return redirect()->route('backend.wisdom.index')->with('error', 'Không tìm thấy dữ liệu');
        }
        $this->data['isEdit'] = 1;
        $this->data['category'] = $this->categoryRepository->getAll(['module_id'=>[ModuleType::Wisdom]]);
        return view('components.backend.wisdom.create', $this->data);
    ...
    public function update( NewsCreateRequest $request, $id )
    {
        ...
        return redirect()->route('backend.wisdom.index')->with('success', 'Đã cập nhật thành công');
    }
    ...
    {
        $params = $request->only(['username', 'password']);
        $post = $this->categoryRepository->getAll(['module_id'=>[ModuleType::Wisdom]], 20);
        ...
        $page = !empty($request->page) ? $request->page : 1;
        $url = route('backend.wisdom.category') . '?' . Arr::query($params);
        $this->data['pager'] = PaginationHelper::BackendPagination($total, $perPage, $page, $url);
        return view('components.backend.wisdom.category', $this->data);
    }
    ...
    public function createCategory()
    {
        $this->data['category'] = $this->categoryRepository->getAll(['module_id'=>[ModuleType::Wisdom]]);
        $this->data['isEdit'] = 0;
        return view('components.backend.wisdom.category-create', $this->data);
    }
    ...
    public function storeCategory( CategoryRequest $request )
    {
        ...
        if ( !$post ) {
            return redirect()->route('backend.wisdom.category')->with('error', 'Server đang bận không thể tạo');
        }

        return redirect()->route('backend.wisdom.category')->with('success', 'Đã tạo tài thành công');
    }
    ...
    public function updateCategory( CategoryRequest $request, $id )
    {
        ...
        if ( !$post ) {
            return redirect()->route('backend.wisdom.category')->with('error', 'Không tìm thấy dữ liệu');
        }
        $post->update($params);

        return redirect()->route('backend.wisdom.category')->with('success', 'Đã cập nhật thành công');
    }
