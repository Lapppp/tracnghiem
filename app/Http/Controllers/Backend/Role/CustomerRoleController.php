...
class CustomerRoleController extends BackendController
{
    ...
    public function index()
    {
        ...
        return view('components.backend.customer-role.index', $this->data);
    }
    ...
    public function create(Request $request)
    {
        ...
        $this->data['permissions'] = $this->permissionRepository->getAll(['parent_id'=>[0],'guard_name'=>'web']);
        return view('components.backend.customer-role.create', $this->data);
    }
    ...
    public function store(RoleCreateRequest $request)
    {
        ...
        return redirect()->route('backend.customer_role.index')->with('success', 'Đã tạo vai trò thành công');
    }
    ...
    public function edit($id)
    {
        $admin = Role::find($id);
        if ( !$admin ) {
            return redirect()->route('backend.customer_role.index')->with('error', 'Không tìm thấy dữ liệu');
        }
