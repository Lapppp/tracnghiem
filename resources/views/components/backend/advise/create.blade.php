<x-layout.backend>
    <div class="card mb-5 w-lg-1000px">
        ...
        <div class="card-body pt-9 pb-0">
            <form id="kt_modal_new_target_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" enctype="multipart/form-data" action="{{ $isEdit == 0 ? Route('backend.advise.store') : Route('backend.advise.update',['id'=>$posts->id]) }}" method="post">
                @if($isEdit == 1)
                    @method('PUT')
                @endif
                @csrf
                ...
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    ...
                    <!--end::Label-->
                    <input type="text" class="form-control form-control-solid" placeholder="Nhập họ tên" name="full_name" id="full_name" value="{{ old('name',!empty($posts) ? $posts->name : '') }}">
                    @if ($errors->has('full_name'))
                        <div class="fv-plugins-message-container invalid-feedback">
                            <div data-field="name" data-validator="notEmpty">{{ $errors->first('full_name') }}</div>
                        </div>
                    @endif
                </div>
                <!--end::Input group-->
