    ...
    <div class="card mb-5 mb-xl-8">
        ...
        <div class="card-header border-0 pt-5">
            ...
            <div class="card-toolbar">
                @if(Auth::guard('backend')->user()->can(['banner_create']))
                <a href="{{ Route('backend.banner.create') }}" class="btn btn-sm btn-light-primary">
            ...
            <div class="table-responsive">
                ...
                <table class="table align-middle gs-0 gy-4">
                    ...
                    <tr class="fw-bolder text-muted bg-light">
                        <th class="ps-4 min-w-325px rounded-start">Tên tập tin</th>
                        ...
                        @foreach($items as $item)
                            <tr>
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="symbol symbol-50px me-5">
                                            @if($item->default() && $item->default()['url'])
                                                <img src="{{ str_replace(Str::of($item->default()['url'])->basename(),'thumb_50x50_'.Str::of($item->default()['url'])->basename(),asset('storage/banner/'.$item->default()['url'])) }}"
                                                     class=""
                                                     alt=""/>
                                            @else
                                        ...
                                        <div class="d-flex justify-content-start flex-column">
                                            <a href="{{ Route('backend.banner.edit',['id'=>$item->id]) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{ $item->name }}</a>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="text-end">
                                    ...
                                    @if(Auth::guard('backend')->user()->can(['banner_edit']))
                                    <a href="{{ Route('backend.banner.edit',['id'=>$item->id]) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Sửa" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    @endif

                                    @if(Auth::guard('backend')->user()->can(['banner_delete']))
