<x-layout.backend>
    <div class="card mb-5 w-lg-700px">
        ...
        <div class="card-body pt-9 pb-0">
            <form id="kt_modal_new_target_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ $isEdit == 0 ? Route('backend.customer_role.store') : Route('backend.customer_role.update',['id'=>$admin->id]) }}" method="post">
                @if($isEdit == 1)
                    @method('PUT')
                @endif
                @csrf
