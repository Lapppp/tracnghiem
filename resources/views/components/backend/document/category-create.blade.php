<x-layout.backend>
    <div class="card mb-5 w-lg-1000px">
        ...
        <div class="card-body pt-9 pb-0">
            <form id="kt_modal_new_target_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" enctype="multipart/form-data" action="{{ $isEdit == 0 ? Route('backend.document.category.store') : Route('backend.document.category.update',['id'=>$posts->id]) }}" method="post">
                @if($isEdit == 1)
                    @method('PUT')
                @endif
                @csrf
                ...
                <!--end::Input group-->
                    ...
                    <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                        ...
                        <label class="d-flex align-items-center fs-6 fw-bold mb-2">
                            ...
                            <i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="tooltip" title="" data-bs-original-title="Chọn danh mục theo từng dịch vụ" aria-label="Specify a target name for future usage and reference"></i>
                        </label>
                        <!--end::Label-->
                        <select class="form-select form-select-solid" data-control="select2" data-placeholder="Chọn vai trò" name="parent_id" data-select2-id="select2-data-10-gou0" tabindex="-1" aria-hidden="true">
                            <option value="0">Chọn danh mục (Mặc định là danh mục cha)</option>
                            @foreach ($category as $key => $value)
                                <option value="{{ $value->id }}" {{ old('status',!empty($posts) ? $posts->parent_id : '') == $value->id ? "selected" : "" }}>{{ $value->name }}</option>
                            @endforeach
                        </select>
