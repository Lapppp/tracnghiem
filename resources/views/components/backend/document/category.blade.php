    ...
    <div class="card mb-5 mb-xl-8">
        ...
        <div class="card-header border-0 pt-5">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ Route('backend.document.category.create') }}" class="btn btn-sm btn-light-primary">
            ...
            <div class="table-responsive">
                ...
                <table class="table align-middle gs-0 gy-4">
                    ...
                    <tr class="fw-bolder text-muted bg-light">
                        <th class="ps-4 min-w-325px rounded-start">Tiêu đề</th>
                    ...
                    @if(count($items) > 0)
                        @foreach($items as $item)
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="d-flex justify-content-start flex-column">
                                            @if(Auth::guard('backend')->user()->can(['edit_category_document']))
                                                <a href="{{ Route('backend.document.category.edit',['id'=>$item->id]) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{ $item->name }}</a>
                                            @else
                                ...
                                <td>
                                    @if($item->status == 1)
                                        <span class="badge badge-light-primary fs-7 fw-bold">{{ $statusCategory[$item->status] }}</span>
                                    @else
                                        <span class="badge badge-light-danger fs-7 fw-bold">{{ $statusCategory[$item->status] }}</span>
                                    @endif
                                </td>
                                <td class="text-end">

                                    @if(Auth::guard('backend')->user()->can(['edit_category_document']))
                                    <a href="{{ Route('backend.document.category.edit',['id'=>$item->id]) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Sửa" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    @endif

                                    @if(Auth::guard('backend')->user()->can(['delete_category_document']))
                ...
                $(document).on('click','.deleteAction',function(event)
                {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed)
                        {
                            ...
                            $.ajax(
                                {
                                    url: baseUrl+"/document/category/delete/"+id,
                                    type: 'DELETE',
                                    data: {
                                        "id": id,
                                        "_token": token,
