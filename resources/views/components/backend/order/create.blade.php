<x-layout.backend>
    <div class="card mb-5 w-lg-1000px">
        ...
        <div class="card-body pt-9 pb-0">
            <form id="kt_modal_new_target_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" enctype="multipart/form-data" action="{{ $isEdit == 0 ? Route('backend.unit.store') : Route('backend.unit.update',['id'=>$posts->id]) }}" method="post">
                @if($isEdit == 1)
                    @method('PUT')
                @endif
                @csrf
                ...
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    ...
                    <label class="d-flex align-items-center fs-6 fw-bold mb-2">
                        <span class="required">Tên đơn vị tính</span>
                    ...
                    <!--end::Label-->
                    <input type="text" class="form-control form-control-solid" placeholder="Nhập tên đơn vị tính" name="name" id="name" value="{{ old('name',!empty($posts) ? $posts->name : '') }}">
                    @if ($errors->has('name'))
