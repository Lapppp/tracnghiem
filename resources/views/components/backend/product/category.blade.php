    ...
    <div class="card mb-5 mb-xl-8">
        ...
        <div class="card-header border-0 pt-5">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ Route('backend.product.category.create') }}" class="btn btn-sm btn-light-primary">
                        ...
                        @foreach($items as $item)
                            <tr>
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="symbol symbol-50px me-5">
                                            @if($item->default() && $item->default()['url'])
                                                <img src="{{ str_replace(Str::of($item->default()['url'])->basename(),'thumb_50x50_'.Str::of($item->default()['url'])->basename(),asset('storage/category/'.$item->default()['url'])) }}"
                                                     class=""
                                                     alt=""/>
                                            @else
                                        ...
                                        <div class="d-flex justify-content-start flex-column">
                                            <a href="{{ Route('backend.product.category.edit',['id'=>$item->id]) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{ $item->name }}</a>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="text-end">
                                    ...
                                    @if(Auth::guard('backend')->user()->can(['edit_category_product']))
                                    <a href="{{ Route('backend.product.category.edit',['id'=>$item->id]) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Sửa" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    @endif

                                    @if(Auth::guard('backend')->user()->can(['delete_category_product']))
                ...
                $(document).on('click','.deleteAction',function(event)
                {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed)
                        {
                            ...
                            $.ajax(
                                {
                                    url: baseUrl+"/product/category/delete/"+id,
                                    type: 'DELETE',
                                    data: {
                                        "id": id,
                                        "_token": token,
