        ...
        <div class="card-header border-0 pt-5">
            ...
            <div class="card-toolbar">
                @if (Auth::guard('backend')->user()->can(['create_category_questions']))
                    <a href="{{ Route('backend.questions.category.create') }}" class="btn btn-sm btn-light-primary">
                            ...
                            @foreach ($items as $item)
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="symbol symbol-50px me-5">
                                                <img src="{{ Avatar::create($item->name)->toBase64() }}" class=""
                                                    alt="" />
                                            </div>
                                            <div class="d-flex justify-content-start flex-column">
                                                <a href="{{ Route('backend.questions.category.edit', ['id' => $item->id]) }}"
                                    ...
                                    <td>
                                        @if ($item->status == 1)
                                            <span
                                                class="badge badge-light-primary fs-7 fw-bold">{{ $status[$item->status] }}</span>
                                        @else
                                            <span
                                                class="badge badge-light-danger fs-7 fw-bold">{{ $status[$item->status] }}</span>
                                        @endif
                                    </td>
                                    <td class="text-end">

                                        @if (Auth::guard('backend')->user()->can(['edit_category_questions']))
                                            <a href="{{ Route('backend.questions.category.edit', ['id' => $item->id]) }}"
                                        ...
                                        @if (Auth::guard('backend')->user()->can(['delete_category_questions']))
                                            <a href="#" data-bs-toggle="tooltip" data-id="{{ $item->id }}"
                                                data-bs-placement="top" title="Xóa"
                                                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm deleteAction">
                ...
                $(document).on('click', '.deleteAction', function(event) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: baseUrl + "/questions/category/delete/" + id,
                                type: 'DELETE',
                                data: {
                                    "id": id,
                                    "_token": token,
