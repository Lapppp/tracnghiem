    ...
    <div class="card mb-5 w-lg-1000px">
        ...
        <div class="card-body pt-9 pb-0">
            <form id="kt_modal_new_target_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                ...
                action="{{ $isEdit == 0 ? Route('backend.questions.storeCreateEnglish') : Route('backend.questions.updateCreateEnglish', ['id' => $posts->id]) }}"
                method="post">
                @if ($isEdit == 1)
                    @method('PUT')
                @endif
                ...
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    ...
                    <!--end::Label-->
                    <textarea rows="5" class="form-control form-control-solid" placeholder="Nhập tên câu hỏi"
                        name="name" id="name">{{ old('name', !empty($posts) ? $posts->name : '') }}</textarea>
                    @if ($errors->has('name'))
                    ...
                    @if(!empty($questions))
                        ...
                        @foreach ($questions as $key => $group)
                            @php
                                ...
                                $c = $group[0]['c'];
                                $d = $group[0]['d'];
                                $is_correct = $group[0]['is_correct'];
                            @endphp
                            <div class="row">
                                <div class="col-md-9">
                                    <div class="mb-10">
                                        <label for="exampleFormControlInput_{{ $id }}_a" data-id="{{ $id }}_a" class="form-label labelAnswerAjax">A</label>
                                        <input type="text" class="form-control form-control-solid" name="answers[{{$key}}][]" value="{{ $a ?? '' }}"
                                ...
                                <div class="col-md-3">
                                    <div class="form-check form-check-custom form-check-solid mt-5 pt-5">
                                        <input class="form-check-input valueCorrect" type="radio"  name="correct[{{$key}}][]"
                            ...
                            <div class="row">
                                <div class="col-md-9">
                                    <div class="mb-10">
                                        ...
                                        <input type="text" class="form-control form-control-solid" name="answers[{{$key}}][]" value="{{ $b ?? '' }}"
                            ...
                            <div class="row">
                                <div class="col-md-9">
                                    <div class="mb-10">
                                        ...
                                        <input type="text" class="form-control form-control-solid" name="answers[{{$key}}][]" value="{{ $c ?? '' }}"
                            ...
                            <div class="row">
                                <div class="col-md-9">
                                    <div class="mb-10">
                                        ...
                                        <input type="text" class="form-control form-control-solid" name="answers[{{$key}}][]" value="{{ $d ?? '' }}"
                            ...
                            <div class="row border-top mb-2">
                                <div class="col-md-12">
                                    <input type="hidden" name="groups[{{$key}}]">
                                    <input type="hidden" name="is_correct[{{$key}}]" id="is_correct_{{ $id }}" value="{{ $is_correct }}">
                                </div>
                            </div>
                        @endforeach
                    @else
                        @include('components.backend.quiz.questions.answerEnglish')
                    @endif
                ...
                @if ($isEdit == 0)
                    <div class="d-flex flex-column mb-8">
                        <div class="col-md-6 fv-row fv-plugins-icon-container">
                            <a href="#" class="btn btn-success" data-id="0" id="addAnswerEnglish"><i class="bi bi-file-plus fs-4 me-2"></i>Thêm câu trả lời</a>
                        </div>
                    </div>
                @endif
                ...
                @if ($isEdit == 1)
                    <div class="d-flex flex-column mb-8">
                        <div class="col-md-6 fv-row fv-plugins-icon-container">
                            <a href="#" class="btn btn-success" data-id="{{ $posts->id }}" id="addAnswerEnglish"><i class="bi bi-file-plus fs-4 me-2"></i>Thêm câu trả lời</a>
                        </div>
                    </div>
                @endif
    ...
    <x-slot name="javascript">
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $(document).on('click', '#addAnswerEnglish', function(event) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ Route('backend.questions.addQuestionEnglish') }}",
                        dataType: 'json',
                        data: {
                            "_token": token,
                            post_id: post_id,
                        ...
                        success: function(jsonResultHtml) {
                            $('#list-answers').append(jsonResultHtml.data.responseHtml)
                        }
                    });
