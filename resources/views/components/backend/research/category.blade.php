    ...
    <div class="card mb-5 mb-xl-8">
        ...
        <div class="card-header border-0 pt-5">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ Route('backend.research.category.create') }}" class="btn btn-sm btn-light-primary">
                        ...
                        @foreach($items as $item)
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="d-flex justify-content-start flex-column">
                                            <a href="{{ Route('backend.research.category.edit',['id'=>$item->id]) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{ $item->name }}</a>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="text-end">
                                    ...
                                    @if(Auth::guard('backend')->user()->can(['edit_category_research']))
                                    <a href="{{ Route('backend.research.category.edit',['id'=>$item->id]) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Sửa" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    @endif

                                    @if(Auth::guard('backend')->user()->can(['delete_category_research']))
                ...
                $(document).on('click','.deleteAction',function(event)
                {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed)
                        {
                            ...
                            $.ajax(
                                {
                                    url: baseUrl+"/research/category/delete/"+id,
                                    type: 'DELETE',
                                    data: {
                                        "id": id,
                                        "_token": token,
