            ...
            <div class="card-toolbar">
                ...
                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-bold w-200px" data-kt-menu="true">
                    ...
                    @if(Auth::guard('backend')->user()->can(['create_role']))
                        <div class="menu-item px-3">
                            <a href="{{ Route('backend.role.create') }}" class="menu-link px-3">Tạo vai trò</a>
                        </div>
                    @endif
                    <!--end::Menu item-->
                    ...
                    @if(count($roles) > 0)
                        @foreach($roles as $role)
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="d-flex justify-content-start flex-column">
                                            <a href="{{ Route('backend.role.edit',['id'=>$role->id]) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{ $role->customize_name }}</a>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td class="text-end">
                                    @if(Auth::guard('backend')->user()->can(['edit_role']))
                                        <a href="{{ Route('backend.role.edit',['id'=>$role->id]) }}" class="btn btn-bg-light btn-color-muted btn-active-color-primary btn-sm px-4 me-2">Sửa</a>
                                    @endif
                                    @if(Auth::guard('backend')->user()->can(['delete_role']))
