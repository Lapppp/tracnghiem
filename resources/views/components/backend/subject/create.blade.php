    ...
    <div class="card mb-5 w-lg-1000px">
        ...
        <div class="card-body pt-9 pb-0">
            <form id="kt_modal_new_target_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                ...
                action="{{ $isEdit == 0 ? Route('backend.subject.store') : Route('backend.subject.update', ['id' => $posts->id]) }}"
                method="post">
                @if ($isEdit == 1)
                    @method('PUT')
                @endif
                ...
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    ...
                    <label class="d-flex align-items-center fs-6 fw-bold mb-2">
                        <span class="required">Tên topic</span>
                        <i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="tooltip" title=""
                            data-bs-original-title="Tên của một Tên topic"
                            aria-label="Specify a target name for future usage and reference"></i>
                    </label>
                    <!--end::Label-->
                    <input type="text" class="form-control form-control-solid" placeholder="Nhập tên Tên topic"
                        name="title" id="title" value="{{ old('title', !empty($posts) ? $posts->title : '') }}">
                    @if ($errors->has('title'))
