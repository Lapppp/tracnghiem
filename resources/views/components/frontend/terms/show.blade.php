        ...
        <div class="ctn">
            ...
            <div class="bread">
                <ul>
                    ...
                    <li><a href="{{ Route('frontend.terms.index') }}">{{ $post->category->name ?? '' }}</a></li>
                    <li>{{ $post->name ?? '' }}</li>
                    ...
                    <div class="dmsibar">
                        ...
                        <ul>
                            @foreach($category as $key =>$item)
                                <li>
                                    <a href="{{ Route('frontend.news.category',['id'=>$item->id,'name'=>Str::slug($item->name.'', '-')]) }}">
                                        {{ $item->name ?? '' }}
                                    </a>
                                </li>
                            @endforeach
                    ...
                    <div class="dmsibar">
                        ...
                        <ul>
                            @foreach($otherNews as $key => $value)
                                <li>
                                    <a href="{{ Route('frontend.news.show',['id'=>$value->id,'name'=>Str::slug($value->name.'', '-')]) }}">{{ $value->name }}</a>
                                </li>
                            @endforeach
        ...
        <div class="cmd12" id="recent-prd">
            ...
            <div id="sl-recentprd">
                @foreach($otherSameNews as $key => $item)
                    <div class="cmd4">
                        <div class="it-post">
                            <a href="{{ Route('frontend.terms.show',['id'=>$item->id,'name'=>Str::slug($item->name.'', '-')]) }}">
                            ...
                            <h3><a href="{{ Route('frontend.terms.show',['id'=>$item->id,'name'=>Str::slug($item->name.'', '-')]) }}">{{ $item->name ?? '' }}</a></h3>
